...
class TrainingFlowPipeline_accelerate(Pipeline):
    def  __init__ (self, models: Dict[str, nn.Module] = None, criterion: nn.Module = None, optimizer: optim.Optimizer = None):
        ...
        model, self.optimizer = self.accelerator.prepare(self.models['ss_flow'], self.optimizer)
        self.models['ss_flow'] = model
    ...
    def sampleFromFeature(self, cond : dict, num_samples: int = 1, sampler_params: dict = {}):
        ...
        reso = flow_model.module.resolution # DDP
        noise = torch.randn(num_samples, flow_model.module.in_channels, reso, reso, reso).to(self.device)
